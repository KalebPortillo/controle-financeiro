...
- **RF3.1** Ao cair na inbox, cada transação recebe automaticamente (via job assíncrono após o sync):
  ...
  - **Tags sugeridas** — comportamento depende do estado atual do workspace:
    - **Sem tags existentes (onboarding)**: a IA sugere livremente nomes de tags novas em PT-BR, consistentes entre si. Neste momento ela está ajudando o usuário a construir sua taxonomia do zero, então a sugestão de tag nova é o comportamento esperado e predominante. O usuário aceita, edita o nome ou descarta cada sugestão.
      - **Taxonomia ampla por tema** (não estabelecimentos): as tags sugeridas devem ser temas abrangentes e reutilizáveis — `"Alimentação"`, `"Transporte"`, `"Assinaturas"`, `"Contas da casa"`, `"Saúde"`, `"Lazer"`, `"Mensalidades recorrentes"` — e **nunca** o nome da loja/marca. Ex.: Netflix/Spotify → `"Assinaturas"`; iFood → `"Alimentação"`; Uber/99 → `"Transporte"`. A diretriz vive em `AiProviders::GeminiProvider::TAG_TAXONOMY_GUIDANCE`, compartilhada pelos prompts de onboarding e da inbox.
...
- **RF10.6** **Auto-vínculo por código exato**: muitos estornos (e os de IOF) trazem no nome o **mesmo código/referência** do gasto original. Ao chegar um estorno, o sistema extrai o código distintivo (token alfanumérico com dígito) e procura o gasto correspondente numa janela de datas. Dois regimes:
  - **Estorno comum**: casa pelo código **independente do valor** (pode ser **parcial** — uma compra de R$1.030 volta em pedaços). Alvo = a própria compra.
  - **Estorno de IOF** ("IOF de volta de X CÓDIGO"): o código identifica a compra, mas o alvo é o **débito de IOF daquela compra** (satélite RF23) e o valor precisa ser **exatamente** o do IOF cobrado antes.
  Se **exatamente um** gasto casa, **aplica automaticamente** (`origin: automatic`, sem confirmação), reduz o valor consolidado (RF10.3), **notifica** (in-app + Telegram) e deixa **desfazer** fácil (selo "vinculado automaticamente"). Se o código casa **mais de um** gasto, o nome é **genérico**, ou o IOF não tem satélite de valor exato, fica **desagrupado** e vira apenas **sugestão** no detalhe. Roda no sync (após dados novos) e via backfill `rails refunds:autolink`. Idempotente.
