...
type Source = Pick<
  InboxTransaction,
  'account_kind' | 'institution_label' | 'account_name' | 'account_institution_name' | 'account_brand' | 'account_last_digits'
>

/**
 * Fonte do gasto (RF2.7): ícone + banco + cartão/conta —
 *   cartão: "Nubank · Mastercard 1234" (banco · bandeira dígitos)
 *   conta:  "Inter · conta corrente"
 * Banco = nome real do conector (account_institution_name); cai para o
 * institution_label e depois o account_name quando faltar.
 */
export function AccountTag({ t, size = 12 }: { t: Source; size?: number }) {
  ...
  if (t.account_kind === 'credit_card') {
    ...
    detail = t.account_last_digits ? `${brand} ${t.account_last_digits}` : brand
