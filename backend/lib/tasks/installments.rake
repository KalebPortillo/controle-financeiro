namespace :installments do
  ...
  #   - remove parcelas projetadas duplicadas (purchaseDate sintético sem MCC)
  #     quando há a parcela canônica.
  # Idempotente. Uso: bin/rails installments:regroup
  desc "Reagrupa parcelamentos pela chave purchaseDate e remove duplicatas projetadas"
  task regroup: :environment do
    result = Transactions::RegroupInstallments.call
    puts "[installments:regroup] regrouped=#{result[:regrouped]} removed=#{result[:removed]}"
  end
end
